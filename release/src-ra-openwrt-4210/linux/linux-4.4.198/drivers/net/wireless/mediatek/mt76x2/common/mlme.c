...
#ifdef ED_MONITOR
	if ((pAd->ed_chk != EDCCA_OFF) && pAd->ed_timer_inited)
		ed_status_read(pAd);
#ifdef ED_SMART
...
#define WPS_AP		0x01
	...
	if (IS_ADPTR_EZ_SETUP_ENABLED(pAd)) {
		if (ie_list->vendor_ie.support_easy_setup) {
		...
		ez_update_bss_entry(pBss, &(ie_list->vendor_ie));
#endif
						}
#endif /* WH_EZ_SETUP */
}
