		...
		IN UINT32 Flag)
{
	...
	if (pMbss->pRoutingEntryPool == NULL)
		...
		sizeof(struct _ROUTING_ENTRY) * ROUTING_POOL_SIZE);

	if (pMbss->pRoutingEntryPool) {
		NdisZeroMemory(pMbss->pRoutingEntryPool, sizeof(struct _ROUTING_ENTRY) * ROUTING_POOL_SIZE);
		...
		IN UINT32 Flag)
{
	INT32 i;
	MULTISSID_STRUCT *pMbss = NULL;
	struct _ROUTING_ENTRY *pRoutingEntry = NULL, *pRoutingEntryNext = NULL;
		...
		while (pRoutingEntry) {
			...
			if (pRoutingEntry->Valid && ROUTING_ENTRY_TEST_FLAG(pRoutingEntry, Flag)) {
				...
				if (!IS_ROUTING_ENTRY(pRoutingEntry)) {
					...
					NdisZeroMemory(pRoutingEntry, sizeof(struct _ROUTING_ENTRY));
		...
		IN UCHAR ifIndex)
{
	...
	RTMP_SEM_LOCK(&pMbss->RoutingTabLock);
	pRoutingEntry = (struct _ROUTING_ENTRY *)removeHeadList(&pMbss->RoutingEntryFreeList);
	RTMP_SEM_UNLOCK(&pMbss->RoutingTabLock);
		...
		IN INT32 Index)
{
	...
	if (Index < ROUTING_HASH_TAB_SIZE)
		return (struct _ROUTING_ENTRY *)pMbss->RoutingTab[Index].pHead;
	else
		return NULL;
}
