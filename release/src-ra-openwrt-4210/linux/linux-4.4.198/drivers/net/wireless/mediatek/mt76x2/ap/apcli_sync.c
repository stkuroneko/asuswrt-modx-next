	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (IS_ADPTR_EZ_SETUP_ENABLED(pAd)
		...
		bss_idx = BssTableSearchWithSSID(&pAd->ScanTab, (PCHAR)Info->Bssid,
			(PCHAR)Info->Ssid, Info->SsidLen, pApCliEntry->MlmeAux.Channel);
		if (bss_idx != BSS_NOT_FOUND) {
			...
			EZ_DEBUG(DBG_CAT_CLIENT, CATCLIENT_APCLI, EZ_DBG_LVL_TRACE,
				("BssEntry Not found in pAd->ScanTab\n"));
			*pCurrStateCtrl = APCLI_CTRL_DISCONNECTED;
			RTMPCancelTimer(&(pApCliEntry->MlmeAux.ProbeTimer), &cancelled);
			return;
		}
		EZ_DEBUG(DBG_CAT_CLIENT, CATCLIENT_APCLI, EZ_DBG_LVL_TRACE,
			("EasySetup Conn = %d\n",
			pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.support_easy_setup));
	} else if (IS_EZ_SETUP_ENABLED(&pApCliEntry->wdev) &&
		(pWpsCtrl->WscConfMode != WSC_DISABLE) &&
		(pWpsCtrl->bWscTrigger == TRUE)) {
		ULONG bss_idx = BSS_NOT_FOUND;

		bss_idx = BssTableSearchWithSSID(&pAd->ScanTab,
			(PCHAR)Info->Bssid, (PCHAR)Info->Ssid, Info->SsidLen,
			pApCliEntry->MlmeAux.Channel);
		if (bss_idx != BSS_NOT_FOUND) {
			ez_ApCliAutoConnectBWAdjust(pAd, &pApCliEntry->wdev, &pAd->ScanTab.BssEntry[bss_idx]);
			ez_ApCliAutoConnectBWAdjust(pAd,
				&pAd->ApCfg.MBSSID[pApCliEntry->wdev.func_idx].wdev,
								...
								pVIE))
	{
		...
		*/
		...
		if (Bssidx == BSS_NOT_FOUND)
		{
			...
			Bssidx = BssTableSetEntry(pAd, &pAd->ScanTab, ie_list, Rssi, LenVIE,
				pVIE, Elem->Snr0, Elem->Snr1);
#else
		...
		if (IS_EZ_SETUP_ENABLED(wdev) &&
			...
			*/
			if (pFrame->Hdr.FC.SubType == SUBTYPE_BEACON)
				...
				ez_process_beacon_probe_response(wdev, Elem->Msg, Elem->MsgLen);
		}
#endif /* WH_EZ_SETUP */
		...
		if ((pApCliEntry->WscControl.WscConfMode != WSC_DISABLE) &&
			...
			if(pApCliEntry->WscControl.WscSsid.SsidLength != 0)
				...
				pApCliEntry->WscControl.WscSsid.SsidLength,
		...
		if (matchFlag)
		{
			...
			if (ie_list->vendor_ie.mtk_cap_found) {
				if (ie_list->vendor_ie.support_mwds) {
					pAd->ScanTab.BssEntry[Bssidx].bSupportMWDS = TRUE;
	...
	IN USHORT ifIndex)
{
	...
	if (NState != NDIS_STATUS_SUCCESS) {
		...
		if (IS_EZ_SETUP_ENABLED(&pApCliEntry->wdev) &&
			pApCliEntry->MlmeAux.support_easy_setup) {
			FrameLen += ez_build_probe_request_ie(&pApCliEntry->wdev,
				pOutBuffer + FrameLen);
		}
#endif/* WH_EZ_SETUP */
#ifdef WH_EVENT_NOTIFIER
		if (pApCliEntry->wdev.custom_vie.ie_hdr.len > 0) {
			...
			ULONG total_custom_vie_len = sizeof(struct Custom_IE_Header) +
				pApCliEntry->wdev.custom_vie.ie_hdr.len;
...
/* Append WSC information in probe request if WSC state is running */
		...
		if ((pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP != WPA_SUPPLICANT_DISABLE) &&
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &ExtraIeTmpLen,
				pAd->cfg80211_ctrl.ExtraIeLen, pAd->cfg80211_ctrl.pExtraIe, END_OF_ARGS);
