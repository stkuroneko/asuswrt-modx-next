...
#ifdef RT_BIG_ENDIAN
	NdisMoveMemory(temp_header, dot11_hdr, 40);
	RTMPFrameEndianChange(NULL, temp_header, DIR_READ, TRUE);
	pHeader = temp_header;
#endif
	if (L2PAD) {
		/* For RLT MAC data frame, MAC may add 2 bytes PAD to the tail of 802.11 header.*/
		/* |****802.11 header*****|**PAD**|**LLC-SNAP**|*****data*****|*/
		/* For sniffer mode, the PAD must be removed. */
		if (pHeader->FC.Type == 0x2 /* FC_TYPE_DATA */) {
			...
			/* Skip 2 bytes L2PAD. */
			NdisMoveMemory(pData+2, pData, header_len);
			skb_pull(pOSPkt, 2);
		}
	}
